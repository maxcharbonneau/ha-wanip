...
class WANIPApiClientError(Exception):
    """Base exception to indicate a general API error."""


class WANIPApiClientCommunicationError(
    WANIPApiClientError,
):
    """Exception to indicate a communication error with the API."""


class WANIPApiClientAuthenticationError(
    WANIPApiClientError,
):
...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        WANIPApiClientAuthenticationError: For 401/403 errors.
        aiohttp.ClientResponseError: For other HTTP errors.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise WANIPApiClientAuthenticationError(
            msg,
        )
    response.raise_for_status()


class WANIPApiClient:
    """
    ...
    async def async_get_data(self) -> Any:
        ...
        Raises:
            WANIPApiClientAuthenticationError: If authentication fails.
            WANIPApiClientCommunicationError: If communication fails.
            WANIPApiClientError: For other API errors.
    ...
    async def _api_wrapper(
        ...
        try:
            ...
            raise WANIPApiClientCommunicationError(
                msg,
            ) from exception
