...
- **`entity/`** - Base entity classes (inherit `WANIPEntity` from `entity/base.py`)
...
**MUST inherit from:** `(PlatformEntity, WANIPEntity)` - order matters for MRO
...
**Imports pattern:** `from homeassistant.components.PLATFORM import PlatformEntity, PlatformEntityDescription` + `from ..entity import WANIPEntity`
