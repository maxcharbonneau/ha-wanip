...
**Directory:** `custom_components/wanip/[platform]/`
...
from .entity import WANIPEntity
from .[entity_file] import WANIP[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: WANIPDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            WANIP[EntityName](coordinator, entry),
...
Add platform to `custom_components/wanip/manifest.json`:
...
- [ ] Entity class inherits from both `WANIPEntity` and platform class
...
- **Domain:** `wanip`
- **Class prefix:** `WANIP`
- **Base entity:** `WANIPEntity` in `entity/base.py`
- **Coordinator:** `WANIPDataUpdateCoordinator`
