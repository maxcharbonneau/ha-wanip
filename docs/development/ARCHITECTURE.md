...
This document describes the technical architecture of the WANIP custom component for Home Assistant.

## Directory Structure

```text
custom_components/wanip/
...
│   ├── __init__.py          # Exports WANIPDataUpdateCoordinator
...
│   ├── __init__.py          # Exports WANIPEntity
...
- `base.py` - Main coordinator class (`WANIPDataUpdateCoordinator`)
...
**Key class:** `WANIPDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `WANIPConfigFlowHandler` (main flow)
- `WANIPOptionsFlow` (options)
...
**Key class:** `WANIPEntity` (in `entity/base.py`)
...
2. `WANIPEntity` for common functionality
...
1. Create directory: `custom_components/wanip/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `WANIPEntity`
