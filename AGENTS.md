...
- **Domain:** `wanip`
- **Title:** WANIP
- **Repository:** maxcharbonneau/ha-wanip

**Key directories:**

- `custom_components/wanip/` - Main integration code
...
- Integration logs: `custom_components.wanip: debug` in `config/configuration.yaml`
...
- Use the domain `wanip` for all DOMAIN references
- Prefix all integration-specific classes with `WANIP`
- Use "WANIP" as the display title
...
- Inherit from platform base + `WANIPEntity`
...
- `tests/` mirrors `custom_components/wanip/` structure
